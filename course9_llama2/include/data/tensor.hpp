...
class Tensor<float> {
 public:
  ...
  explicit Tensor(float *raw_ptr, uint32_t size);

  explicit Tensor(float *raw_ptr, uint32_t rows, uint32_t cols);

  explicit Tensor(float *raw_ptr, uint32_t channels, uint32_t rows, uint32_t cols);

  explicit Tensor(float *raw_ptr, const std::vector<uint32_t> &shapes);
