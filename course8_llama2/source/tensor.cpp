...
Tensor<float>::Tensor(float* raw_ptr, uint32_t size) {
  CHECK_NE(raw_ptr, nullptr);
  this->raw_shapes_ = {size};
  this->data_ = arma::Cube<float>(raw_ptr, 1, size, 1, false, true);
}

Tensor<float>::Tensor(float* raw_ptr, uint32_t rows, uint32_t cols) {
  CHECK_NE(raw_ptr, nullptr);
  this->data_ = arma::Cube<float>(raw_ptr, rows, cols, 1, false, true);
  if (rows == 1) {
...
Tensor<float>::Tensor(float* raw_ptr, uint32_t channels, uint32_t rows, uint32_t cols) {
  CHECK_NE(raw_ptr, nullptr);
  this->data_ = arma::Cube<float>(raw_ptr, rows, cols, channels, false, true);
...
Tensor<float>::Tensor(float* raw_ptr, const std::vector<uint32_t>& shapes) {
